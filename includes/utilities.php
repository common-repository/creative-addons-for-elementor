...
class Utilities {
	...
	public static function user_not_logged_in() {
		self::ajax_show_error_die( '<p>' . esc_html__( 'You are not logged in. Refresh your page and log in.', 'creative-addons-for-elementor' ) . '</p>', esc_html__( 'Cannot save your changes', 'creative-addons-for-elementor' ) );
	}
	...
	public static function save_wp_option( $option_name, $option_value ) {
		...
		if ( is_array( $option_value ) || is_object( $option_value ) ) {
			...
			if ( empty( $serialized_value ) ) {
				return new WP_Error( '434', esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' ' . $option_name );
			}
		}
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( ! self::is_positive_int( $post_id) ) {
			return $return_error ? new WP_Error( esc_html__( 'Error occurred', 'creative-addons-for-elementor' ), self::get_variable_string( $post_id ) ) : $default;
		}
	...
	public static function save_postmeta( $post_id, $meta_key, $meta_value, $sanitized ) {
		...
		if ( ! self::is_positive_int( $post_id) ) {
			return new WP_Error( esc_html__( 'Error occurred', 'creative-addons-for-elementor' ), self::get_variable_string( $post_id ) );
		}

		if ( $sanitized !== true ) {
			return new WP_Error( '433', esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' ' . $meta_key );
		}
		...
		if ( is_array( $meta_value ) || is_object( $meta_value ) ) {
			...
			if ( empty($serialized_value) ) {
				return new WP_Error( '434', esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' ' . $meta_key );
			}
		}
		...
		if ( empty($result) ) {
			if ( false === $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->postmeta (`meta_key`, `meta_value`, `post_id`) VALUES (%s, %s, %d)", $meta_key, $serialized_value, $post_id ) ) ) {
				...
				return new WP_Error( '33', esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) );
			}
		} else {
		...
		if ( $result === false ) {
			...
			return new WP_Error( '435', esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' ' . $meta_key );
		}
	...
	public static function send_email( $message, $to_support_email='', $reply_to_email='', $reply_to_name='', $subject='' ) {
		...
		if ( empty( $message ) || strlen( $message ) > 5000 ) {
			...
			return esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' (0)';
		}
		...
		if ( empty( $to_support_email ) || strlen( $to_support_email) > 100 ) {
			return esc_html__( 'Invalid email', 'creative-addons-for-elementor' ) . ' (1)';
		}

		if ( ! is_email( $to_support_email ) ) {
			return esc_html__( 'Invalid email', 'creative-addons-for-elementor' ) . ' (2)';
		}
		...
		if ( empty( $reply_to_email ) || strlen( $reply_to_email ) > 100 ) {
			return esc_html__( 'Invalid email', 'creative-addons-for-elementor' ) . ' (3)';
		}
		
		if ( ! is_email( $reply_to_email ) ) {
			return esc_html__( 'Invalid email', 'creative-addons-for-elementor' ) . ' (4)';
		}
		
		if ( strlen( $reply_to_name ) > 100 ) {
			return esc_html__( 'Error occurred', 'creative-addons-for-elementor' ) . ' (5)';
		}
		...
		if ( strlen( $subject ) > 200 ) {
			return esc_html__( 'Invalid subject', 'creative-addons-for-elementor' );
		}
		
		if ( strlen( $message ) > 10000 ) {
			return esc_html__( 'Email message is too long', 'creative-addons-for-elementor' );
		}
		...
		return $result == false ? esc_html__( 'Failed to send the email.', 'creative-addons-for-elementor' ) : '';
	}
}
