...
class Admin_Handlers {
	...
	public function save_widgets() {
		...
		if ( empty( $_REQUEST['crel_settings_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_REQUEST['crel_settings_nonce'] ) ) ) ), 'crel_settings_nonce' ) ) {
			wp_die( wp_json_encode( array( 'status' => 'error', 'message' => esc_html__( 'You do not have permission to edit these settings', 'creative-addons-for-elementor' ) ) ) );
		}
		
		$inactive_widgets = Utilities::post('inactive_widgets', [] );
		Widgets_Manager::set_inactive_widgets( $inactive_widgets );
		
		wp_die( wp_json_encode( array( 'status' => 'success', 'message' => esc_html__( 'Widgets were saved', 'creative-addons-for-elementor' ) ) ) );
	}
	...
	public function switch_to_globals() {
		...
		// verify that request is authentic
		if ( empty( $_REQUEST['crel_settings_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_REQUEST['crel_settings_nonce'] ) ) ) ), 'crel_settings_nonce' ) ) {
			wp_die( wp_json_encode( array( 'status' => 'error', 'message' => esc_html__( 'You do not have permission to make a change', 'creative-addons-for-elementor' ) ) ) );
		}

		$switch_to_globals = Utilities::post( 'switch_to_globals' );
		if ( $switch_to_globals == '' ) {
			wp_die( wp_json_encode( array( 'status' => 'error', 'message' => esc_html__( 'Please reload the page', 'creative-addons-for-elementor' ) ) ) );
		}
		...
		wp_die( wp_json_encode( array( 'status' => 'success', 'message' => esc_html__( 'Switch to globals completed. Please update your pages as required.', 'creative-addons-for-elementor' ) ) ) );
	}
	...
	public function save_presets() {
		...
		wp_die( wp_json_encode( array( 'status' => 'success', 'message' => esc_html__( 'Presets were saved', 'creative-addons-for-elementor' ) ) ) );
	}
	...
	public function toggle_debug() {
		...
		// verify that request is authentic
		if ( ! isset( $_REQUEST['_wpnonce_crel_toggle_debug'] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce_crel_toggle_debug'] ) ) ) ), '_wpnonce_crel_toggle_debug' ) ) {
			Utilities::ajax_show_error_die( esc_html__( 'Refresh your page', 'creative-addons-for-elementor' ) );
		}

		// ensure user has correct permissions
		if ( ! current_user_can( 'manage_options' ) ) {
			Utilities::ajax_show_error_die( esc_html__( 'You do not have permission.', 'creative-addons-for-elementor' ) );
		}
		...
		Utilities::save_wp_option( Admin_Handlers::CREL_DEBUG, $is_debug_on );

		// we are done here
		$msg = $is_debug_on ? esc_html__( 'Debug is now on', 'creative-addons-for-elementor' ) : esc_html__( 'Debug is now off', 'creative-addons-for-elementor' );

		Utilities::ajax_show_info_die( $msg );
	}
	...
	public function download_debug_info() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_crel_download_debug_info'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce_crel_download_debug_info'] ) ) ) ), '_wpnonce_crel_download_debug_info' ) ) {
		...
		Utilities::save_wp_option( Admin_Handlers::CREL_DEBUG, false );
	...
	public function user_not_logged_in() {
		Utilities::ajax_show_error_die( '<p>' . esc_html__( 'You are not logged in. Refresh your page and log in.', 'creative-addons-for-elementor' ) . '</p>', esc_html__( 'Cannot save your changes', 'creative-addons-for-elementor' ) );
	}
}