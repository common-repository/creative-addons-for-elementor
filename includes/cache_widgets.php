...
class Cache_Widgets {
	...
	protected $is_built_using_elementor = false;

	/**
	 * Setup cache for given post and its widgets metadata
	 *
    ...
    public function __construct( $post_id=0, $editor_data=null ) {
        ...
        // should not happen
        if ( empty($post_id) || ( ! Utilities::is_published( $post_id ) && ! Utilities::is_private( $post_id ) ) || ! Utilities_Plugin::is_built_using_elementor( $post_id )  ) {
            return;
        }
	    ...
	    $this->is_built_using_elementor = true;
    }
    ...
    public function save() {
		...
		if ( $instance && get_class( Utilities_Plugin::elementor()->db ) == 'Elementor\DB' && method_exists( Utilities_Plugin::elementor()->db, 'iterate_data' ) ) {
	...
	public function get_elementor_editor_data() {
		if ( ! $this->is_built_using_elementor || ! $this->is_published ) {
			return [];
		}

		if ( is_null( $this->elementor_editor_data ) ) {
			$document = Utilities_Plugin::elementor_documents_get( $this->get_post_id() );
