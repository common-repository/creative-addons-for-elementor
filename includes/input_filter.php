...
class Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		if ( empty( $input ) ) {
			return new WP_Error( 'invalid_input', 'Error Occurred' );
		}
		...
		foreach ( $input as $key => $input_value ) {
			...
			$defaults = array(
				'label'       => esc_html__( 'Label', 'creative-addons-for-elementor' ),
				'type'        => self::TEXT,
				'mandatory'   => true,
				'max'         => '20',
				'min'         => '3',
	...
	private function filter_input_field( $value, $field_spec ) {
		...
		switch ( $field_spec['type'] ) {
			...
			default:
				return new WP_Error('eckb-invalid-input-type', 'Error Occurred' . ' - ' . $field_spec['type']);
		}
	}
	...
	private function filter_select( $value, $field_spec ) {
		...
		if ( ! in_array( $value, array_keys($field_spec['options']) )  && ! empty($field_spec['mandatory']) ) {
			$value_text = ( empty($value) ? esc_html__( 'empty', 'creative-addons-for-elementor' ) : '"' . $value . '"' );
			/* translators: 1: value entered by user, 2: HTML tag, 3: valid values user should enter. */
			$msg = '<br>' . sprintf( esc_html__( 'The value cannot be %1$s. %2$sValid values are: %3$s', 'creative-addons-for-elementor' ), $value_text, '<br>', implode(", ", $field_spec['options']) );
	...
	private function filter_checkbox( $value ) {
		...
		/* translators: %s: value entered by user. */
		return new WP_Error('filter_checkbox_invalid', sprintf( esc_html__( 'The value "%s" is not valid', 'creative-addons-for-elementor' ), $value ) );
	}
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		if ( $number != $number_int ) {
			/* translators: %s: value entered by user. */
			return new WP_Error('filter_not_number',sprintf( esc_html__( 'The value "%s" is not a number', 'creative-addons-for-elementor' ), $number ) );
		}

		if ( $number > $field_spec['max'] ) {
			/* translators: 1: value entered by user, 2: maximum length user should enter. */
			$msg = sprintf( esc_html__( 'The value %1$s is larger than maximum of %2$s', 'creative-addons-for-elementor' ), $number, $field_spec['max'] );
			...
			$msg = sprintf( esc_html__( 'The value %1$s is smaller than minimum of %2$s', 'creative-addons-for-elementor' ), $number, $field_spec['min'] );
			return new WP_Error( 'filter_not_number', $msg );
		}
	...
	private function filter_true_false( $boolean ) {
		...
		return new WP_Error( 'filter_not_number', esc_html__( 'Error Occurred', 'creative-addons-for-elementor' ) . ' (112)' );
	}
	...
	public function filter_color_hex( $value, $field_spec=array() ) {
		...
		/* translators: %s: value entered by user. */
		return new WP_Error('filter_not_color_hex', sprintf( esc_html__( 'The value "%s" is not a valid HEX color. Expected format: #xxxxxx', 'creative-addons-for-elementor' ), $value ));
	}
	...
	private function filter_id( $id ) {
		...
		if ( is_wp_error($id) ) {
			return new WP_Error('filter_not_id', esc_html__( 'Error Occurred', 'creative-addons-for-elementor' ) . ' - ' . $id . ' - ' . $id->get_error_code() );
		}
		return $id;
	}
	...
	private function filter_enumeration( $value, $field_spec ) {
		...
		return new WP_Error('filter_not_enumeration', esc_html__( 'Error Occurred', 'creative-addons-for-elementor' ) . ' - ' . $value );
	}
