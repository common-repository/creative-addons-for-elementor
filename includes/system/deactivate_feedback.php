...
class Deactivate_Feedback {
	...
	public function output_deactivate_feedback_dialog() {
		...
		<div id="crel-deactivate-feedback-dialog-wrapper">
			<div id="crel-deactivate-feedback-dialog-header">
				<span id="crel-deactivate-feedback-dialog-header-title"><?php echo esc_html__( 'Quick Feedback', 'creative-addons-for-elementor' ); ?></span>
			</div>
			<form id="crel-deactivate-feedback-dialog-form" method="post">				<?php
				...
				<div id="crel-deactivate-feedback-dialog-form-caption"><?php echo esc_html__( 'If you have a moment, please share why you are deactivating Creative Add-ons:', 'creative-addons-for-elementor' ); ?></div>
				<div id="crel-deactivate-feedback-dialog-form-body">
					<div id="crel-deactivate-feedback-dialog-form-error" style="display:none;"><?php echo esc_html__( 'Please Select an Option', 'creative-addons-for-elementor' ); ?></div>					<?php
						...
						foreach ( $deactivate_reasons as $reason_key => $reason ) :		?>
							<div class="crel-deactivate-feedback-dialog-input-wrapper">
								...
								<?php if ( ! empty( $reason['alert'] ) ) : ?>
									<div class="crel-feedback-text"><?php echo esc_html( $reason['alert'] ); ?></div>
								<?php endif; ?>
								...
								<?php if ( ! empty( $reason['contact_me'] ) ) : ?>
									<div class="crel-feedback-checkbox">
										<input id="crel-deactivate-feedback-contact" class="crel-deactivate-feedback-dialog-input" type="checkbox" name="contact_me_<?php echo esc_attr( $reason_key ); ?>" value="yes" /><label for="crel-deactivate-feedback-contact" class="crel-deactivate-feedback-dialog-label"><?php echo esc_html__( 'Contact Me', 'creative-addons-for-elementor' ); ?></label>
									</div>
								<?php endif; ?>
								<?php if ( ! empty( $reason['button'] ) ) : ?>
									<div class="crel-feedback-button"><a class="crel-feedback-button__green" target="_blank" href="<?php echo esc_url( $reason['button']['url'] ); ?>"><?php echo esc_html( $reason['button']['title'] ); ?></a></div>
								<?php endif; ?>
							</div>					<?php
						endforeach; ?>
				</div>
	...
	public function ajax_crel_deactivate_feedback() {
		...
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ) ) ), '_crel_deactivate_feedback_nonce' ) ) {
			wp_send_json_error();
		}
	...
	private function get_deactivate_reasons( $type ) {
		...
		switch ( $type ) {
		   case 1:
		   	$deactivate_reasons = [
				  'missing_feature' => [
					  'title' => esc_html__( 'I\'m missing a feature', 'creative-addons-for-elementor' ),
					  'input_placeholder' => esc_html__( 'Please tell us what is missing', 'creative-addons-for-elementor' ),
					  'contact_me' => true,
				  ],
				  'couldnt_get_the_plugin_to_work' => [
					  'title' => esc_html__( 'I couldn\'t get the plugin to work', 'creative-addons-for-elementor' ),
					  'input_placeholder' => esc_html__( 'Please share the reason', 'creative-addons-for-elementor' ),
					  //'alert' => sprintf( esc_html__( 'We can help you, usually within an hour! If this is our bug, we will give you our %s for free.', 'creative-addons-for-elementor' ), '<a href="' . $pro_link . '" target="_blank">PRO version</a>'),
					  'button' => [
						  'title' => esc_html__( 'Contact us for Help', 'creative-addons-for-elementor' ),
						  'url' => 'https://www.creative-addons.com/technical-support/'
			    ],
					  'contact_me' => true,
			    ],
				  'other' => [
					  'title' => esc_html__( 'Other', 'creative-addons-for-elementor' ),
					  'input_placeholder' => esc_html__( 'Please share the reason', 'creative-addons-for-elementor' ),
					  'button' => [
						  'title' => esc_html__( 'Contact us for Help', 'creative-addons-for-elementor' ),
						  'url' => 'https://www.creative-addons.com/technical-support/'
			...
			default:
				$deactivate_reasons = [
					'no_longer_needed' => [
						'title' => esc_html__( 'I no longer need the plugin', 'creative-addons-for-elementor' ),
						'input_placeholder' => '',
					],
					'missing_feature' => [
					   'title' => esc_html__( 'I\'m missing a feature', 'creative-addons-for-elementor' ),
