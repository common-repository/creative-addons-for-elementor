...
class Cache_Assets {
	...
	public function save() {
		...
		foreach ( $widgets as $index => $widget_name ) {
			...
			// TODO filter CSS
			$file_path = CREATIVE_ADDONS_DIR_PATH . "assets/css/front-end/widgets/{$widget_name}{$suffix}.css";
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
			$css_content = is_readable( $file_path ) && is_file( $file_path ) ?  file_get_contents( $file_path ) : '';
			$styles_content_filtered .= ' /** CSS for widget: ' . $widget_name . ' **/ ' . $css_content;
		...
		file_put_contents( $this->get_post_file_name(), $styles_content_filtered );
	}
