...
class Presets_Handlers {
	...
	public function update_preset() {
		...
		if ( empty( $_REQUEST['crel_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( sanitize_text_field( wp_unslash( $_REQUEST['crel_nonce'] ) ) ) ), 'crel_nonce' ) ) {
		...
		if ( empty( $preset_name ) ) {
			wp_die( wp_json_encode( [ 'status' => 'error', 'message' => esc_html__( 'Preset name is incorrect', 'creative-addons-for-elementor' ) ] ) );
		}

		$widget_type = Utilities::post( 'widget_type' );
		if ( empty( $widget_type ) ) {
			wp_die( wp_json_encode( [ 'status' => 'error', 'message' => esc_html__( 'Cannot save preset', 'creative-addons-for-elementor' ) ] ) );
		}
		...
		foreach ( $all_presets as $p_id => $preset ) {
			...
			if ( ( empty( $preset_id ) || $preset_id != $p_id ) && ! empty( $preset['title'] ) && $preset['title'] == $preset_name && $preset['widget_type'] == $widget_type ) {
				wp_die( wp_json_encode( [ 'status' => 'error', 'message' => esc_html__( 'A preset with this name already exists', 'creative-addons-for-elementor' ) ] ) );
			}
		}
		...
		wp_die( wp_json_encode( [ 'status' => 'success', 'message' => esc_html__( 'Preset saved', 'creative-addons-for-elementor' ), 'new_name' => $preset_name, 'id' => $preset_id, 'preset' => $all_presets[$preset_id] ] ) );
	}
	...
	public function delete_preset() {
		...
		$result = self::update_option( $all_presets );
		if ( is_wp_error( $result ) ) {
			wp_die( wp_json_encode( [ 'status' => 'error', 'message' => esc_html__( 'Cannot delete preset', 'creative-addons-for-elementor' ) ] ) );
		}

		wp_die( wp_json_encode( [ 'status' => 'success', 'message' => esc_html__( 'Preset was removed', 'creative-addons-for-elementor' ) ] ) );
	}
	...
	private static function update_option( $presets ) {
		...
		$result = Utilities::save_wp_option( self::CREL_PRESETS_OPTION, $presets );
	...
	public static function user_not_logged_in() {
		wp_die( wp_json_encode( [ 'status' => 'error', 'message' => esc_html__( 'You do not have permission to edit these settings', 'creative-addons-for-elementor' ) ] ) );
	}
}