...
class KB_Search_Cntrl {
	...
	public function search_kb() {
		...
		if ( is_wp_error( $kb_id ) ) {
			wp_die( wp_json_encode( array( 'status' => 'error', 'search_result' => esc_html__( 'Error occurred. Please try again later.', 'creative-addons-for-elementor' ), 'show_more' => false ) ) );
		}
		...
		if ( empty($search_terms) ) {
			wp_die( wp_json_encode( array( 'status' => 'error', 'search_result' => esc_html( $kb_config['min_search_word_size_msg'] ), 'show_more' => false ) ) );
		}
		...
		if ( empty($result) ) {
			...
			wp_die( wp_json_encode( array( 'status' => 'error', 'search_result' => $search_result, 'show_more' => false ) ) );
			...
			// we are done here
			wp_die( wp_json_encode( array( 'status' => 'success', 'search_result' => $search_result, 'show_more' => $show_more ) ) );
		}
	}
