...
class KB_Handler {
	...
	public static function is_kb_request() {
		...
		$kb_post_type = empty( $_REQUEST['post_type'] ) ? '' : preg_replace('/[^A-Za-z0-9 \-_]/', '', Utilities::request_key( $_REQUEST['post_type'] ) );
		$is_kb_post_type = empty($kb_post_type) ? false : self::is_kb_post_type( $kb_post_type );
		if ( $is_kb_post_type ) {
			return true;
		}

		$kb_taxonomy = empty( $_REQUEST['taxonomy'] ) ? '' : preg_replace('/[^A-Za-z0-9 \-_]/', '', Utilities::request_key( $_REQUEST['taxonomy'] ) );
	...
	public static function load_KB_addons_assets() {
		...
		// ELAY - TODO FUTURE: remove (the function elay_kb_config_load_public_css() is deprecated in ELAY after version 2.14.0); see Knowledge_Base()->call_missed_kb_actions() for ELAY styles
