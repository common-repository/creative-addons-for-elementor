...
class HTML_Elements {
	...
	public function notification_box_basic( $args = array() ) {
		...
		<div <?php echo isset( $args['id'] ) ? 'id="' . esc_attr( $args['id'] ) . '"' : ''; ?>class="crel-notification-box-basic <?php echo 'crel-notification-box-basic--' . esc_attr( $args['type'] ); ?>">

			<div class="crel-notification-box-basic__icon">
				<div class="crel-notification-box-basic__icon__inner crelfa <?php echo esc_attr( $icon ); ?>"></div>
			</div>

			<div class="crel-notification-box-basic__body">
				<h4 class="crel-notification-box-basic__body__title"><?php echo esc_html( $args['title'] ); ?></h4>
				<div class="crel-notification-box-basic__body__desc"><?php echo esc_html( $args['desc'] ); ?></div>
			</div>
	...
	public function advertisement_ad_box( $args ) {
		...
		<div id="<?php echo esc_attr( $args['id'] ); ?>" class="crel-admin-ad-container <?php echo esc_attr( $args['class'] ); ?>">

			<!----- Box Type ----->
			<span class="crel-admin-ad-container__widget"> <i class="crelfa crelfa-puzzle-piece " aria-hidden="true"></i><?php echo esc_html__( 'Plugin', 'creative-addons-for-elementor'); ?></span>

			<!----- Header ----->
			<div class="crel-aa__header-container">
				<div class="crel-header__icon crelfa <?php echo esc_attr( $args['icon'] ); ?>"></div>
				<div class="crel-header__title"><?php echo esc_html( $args['title'] ); ?></div>
			</div>

			<!----- Body ---=--->
			<div class="crel-aa__body-container">
				<div class="featured_img">
					<img class="crel-body__img" src="<?php echo esc_url( $args['img_url'] ); ?>" alt="<?php echo esc_attr( $args['title'] ); ?>">
				</div>
				<p class="crel-body__desc"><?php echo esc_html( $args['desc'] ); ?></p>
				...
				<ul class="crel-body__check-mark-list-container">					<?php
					if ( $args['list'] ) {
						foreach ($args['list'] as $item) {
							...
							echo '<span class="crel-check-mark-list__item__text">' . esc_html( $item ) . '</span>';
							echo '</li>';
						}
					}					?>
				</ul>				<?php
				...
				if ( $args['btn_text'] ) { ?>
					<a href="<?php echo esc_url( $args['btn_url'] ); ?>" target="_blank" class="crel-body__btn crel-body__btn--<?php echo esc_attr( $args['btn_color'] ); ?>"><?php echo esc_html( $args['btn_text'] ); ?></a>				<?php
				}

				if ( !empty( $args['more_info_text'] ) ) { ?>
					<a href="<?php echo esc_url( $args['more_info_url'] ); ?>" target="_blank" class="crel-body__link crel-body__link--<?php echo esc_attr( $args['more_info_color'] ); ?>">
						<span class="crel-body__link__icon crelfa crelfa-info-circle"></span>
						<span class="crel-body__link__text"><?php echo esc_html( $args['more_info_text'] ); ?></span>
