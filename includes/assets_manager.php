...
class Assets_Manager {
	...
	public static function frontend_enqueue_assets( $post_id='' ) {
		...
		if ( Utilities_Plugin::is_built_using_elementor( $post_id ) && ( Utilities::is_published( $post_id ) || Utilities::is_private( $post_id ) ) && ! Utilities::is_post_being_edited() ) {
	...
	public static function enqueue_templates_assets( Post_CSS $file ) {
		...
		if ( empty( $file->get_post_id() ) || empty( get_queried_object_id() ) ||  get_queried_object_id() === $file->get_post_id() ) {
			return;
		}
